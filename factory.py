...
class Factory():
    ...
    def save_encoded_all(self, databank, destination_path=os.path.join(dirname, "train.pt")):
        if os.path.isfile(destination_path):
            with open(destination_path, "rb") as f:
                ...
                if len(databank) > 0 and data[0][-1] != databank[-1]:
    ...
    def save_encoded_with_subject(self, dataRequest):
        ...
        destination_path = os.path.join(
            dirname + "/subject/", dataRequest['subject'] + ".pt")
        ...
        if os.path.isfile(destination_path):
            with open(destination_path, "rb") as f:
                ...
                if len(dataRequest['dataBank']) > 0 and data[0][-1] != dataRequest['dataBank'][-1]:
    ...
    def find_duplicate(self, input_data: list, confident=0.8, subject="train"):
        ...
        if self.data is not None:
            ...
            scores = {self.data[0][idx]: scores[idx].item()
